...
class Train_Scarce_GAN_Normal_Generator:
    ...
    def __init__(self,x_train,y_train,unlabeled_data,num_class,batch,epoch):
        ...
        self.ssgan = Scarce_GAN_Normal_Generator()
               
    
    def forward_pass_discriminator(self,input_data):
        dlayer_output_0 = self.ssgan.discriminator_unsupervised.layers[0].layers[0](input_data)
        dlayer_output_1 = self.ssgan.discriminator_unsupervised.layers[0].layers[1].layers[0](dlayer_output_0)
        dlayer_output_2 = self.ssgan.discriminator_unsupervised.layers[0].layers[1].layers[1](dlayer_output_1)
        dlayer_output_3 = self.ssgan.discriminator_unsupervised.layers[0].layers[1].layers[2](dlayer_output_2)
        dlayer_output_4_1 = self.ssgan.discriminator_unsupervised.layers[0].layers[1].layers[3](dlayer_output_3)
        return dlayer_output_4_1
    ...
    def batch_data_discriminator(self,data_generated):
        data_labeled,data_labeled_y = self.batch_labeled()
        data_unlabeled = self.batch_unlabeled()
        data_unlabeled_y = pd.DataFrame(np.ones((self.batch_size,1))*4)
        data_generated_y = pd.DataFrame(np.ones((self.batch_size,1))*5)
        
        data_1 = pd.DataFrame(data_labeled).append(data_unlabeled)
        data = pd.DataFrame(data_generated).append(data_1)
        
        label_1 = pd.DataFrame(data_labeled_y).append(data_unlabeled_y)
        label = data_generated_y.append(label_1)
        
        return self.stratified_sampling(np.array(data),np.array(label))
    
    def batch_data_supervised_discriminator(self):
        data_labeled,data_labeled_y = self.batch_labeled()
        data_unlabeled = self.batch_unlabeled()
        data_unlabeled_y = pd.DataFrame(np.ones((self.batch_size,1))*4)
        
        data = pd.DataFrame(data_labeled).append(data_unlabeled)
        label = pd.DataFrame(data_labeled_y).append(data_unlabeled_y)
        
        return self.stratified_sampling(np.array(data),np.array(label),self.num_classes+1)
    ...
    def train(self):
        ...
        for iteration in range(iterations):
            ...
            generator_loss = self.ssgan.gan.train_on_batch(z,unsupervised_labels_real)
            ...
            if val_loss[0] < self.best_val_loss:
                ...
                #self.ssgan.discriminator_supervised.save_weights('Model_Weights/Supervised_Discriminator_FM_PT.h5') # OR model.save_weights()
